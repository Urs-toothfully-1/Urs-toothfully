...
export function BranchSwitcher({ currentBranchId, branches }: Props) {
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      >
        {pending ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <MapPin className="h-3.5 w-3.5" style={{ color: BRAND_COLORS.primaryTeal }} />}
            ...
            return (
              ...
              >
                <span className="flex items-center gap-2">
                  <MapPin className="h-3.5 w-3.5" style={{ color: active ? BRAND_COLORS.primaryTeal : BRAND_COLORS.borderDivider }} />
                  {b.name}
                </span>
                {switchingId === b.id ? (
